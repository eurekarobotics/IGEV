...
def train(args):
    best_model = nn.DataParallel(IGEVStereo(args))
    print("Parameter Count: %d" % count_parameters(best_model))
    ...
    if args.restore_ckpt is not None:
        ...
        best_model.load_state_dict(checkpoint, strict=True)
        logging.info(f"Done loading checkpoint")

    ori_param = count_parameters(best_model)
    for it, amount in enumerate(args.amounts):
        model = copy.deepcopy(best_model)
        total_steps = 0

        prev_param = count_parameters(model)
        prune(model, amount, img_size=args.image_size)
        pruned_param = count_parameters(model)

        pruned_ratio = 1 - pruned_param/prev_param

        optimizer, scheduler = fetch_optimizer(args, model)
        logger = Logger(model, scheduler)
        logging.info(f"Pruning iter {it + 1} - amount: {amount} - #params: {prev_param} ==> {pruned_param} ({pruned_ratio * 100}%)")
        model.cuda()
        model.train()
        ...
        while should_keep_training:
            ...
            for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
                ...
                if total_steps % validation_frequency == validation_frequency - 1:
                    ...
                    key_metric = list(results.keys())[0]
                    if best_epe > results[key_metric]:
                        best_epe = results[key_metric]
                        save_path = Path(args.logdir + '/best_pruned_%2.2f_%s.pth' % (1 - pruned_param/ori_param, args.name))
            ...
            if len(train_loader) >= 10000:
                save_path = Path(args.logdir + '/%d_pruned_%2.2f_epoch_%s.pth.gz' % (total_steps + 1, 1 - pruned_param/ori_param, args.name))
                logging.info(f"Saving file {save_path}")
                # torch.save(model.state_dict(), save_path)
                torch.save(model, save_path)

        print(f"FINISHED TRAINING ITER {it + 1}")
        # logger.close()
        # PATH = args.logdir + '/%s_pruned.pth' % (args.name) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--amounts', type=float, nargs='+', default=[0.1, 0.1], help="pruning amounts")
